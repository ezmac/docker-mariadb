...
post_start_action() {
  ...
  mysql -u root <<-EOF
      ...
      GRANT ALL PRIVILEGES ON *.* TO 'tungsten'@'%' IDENTIFIED BY 'password' WITH GRANT OPTION;
      GRANT ALL PRIVILEGES ON *.* TO 'tungsten'@'$(hostname)' IDENTIFIED BY 'password' WITH GRANT OPTION;
EOF
